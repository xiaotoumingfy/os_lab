...
static void *coalesce(void *bp)
{
	...
	PUT(HDRP(PREV_BLKP(bp)), PACK(size, 0));    /*头部修改*/	
	PUT(FTRP(NEXT_BLKP(bp)), PACK(size, 0));    /*尾部修改*/
...
static void place(void *bp, size_t asize)
{
    ...
    /* need split */
    {
	PUT(HDRP(bp),PACK(asize,1)); /*已分配头尾部标识*/
	PUT(FTRP(bp),PACK(asize,1));
	bp = NEXT_BLKP(bp);
	PUT(HDRP(bp),PACK(rest,0)); /*未分配头尾部标识*/
	PUT(FTRP(bp),PACK(rest,0));
        /* 已补全 */
    }
    else
    {
	PUT(HDRP(bp),PACK(total_size,1)); /*已分配头尾部标识*/
